...
@bot.on_message(command(['pause', 'resume']) & private & user(Var.ADMINS))
async def toggle_fetch(client, message):
    action = message.command[0]
    ani_cache['fetch_animes'] = action == 'resume'
    await sendMessage(message, f"`Successfully {'Resumed' if action == 'resume' else 'Paused'} Fetching Animes...`")
